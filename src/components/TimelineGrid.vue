...
// === Edge auto-scroll related state ===
...
// ===================================================================================
// Operator selection dialog logic
// ===================================================================================
...
const ELEMENT_FILTERS = [
  ...
  { label: 'Heat', value: 'blaze', color: '#ff4d4f' },
  { label: 'Cryo', value: 'cold', color: '#00e5ff' },
  { label: 'Electric', value: 'emag', color: '#ffd700' },
...
// ===================================================================================
// Core logic: operation axis calculation
// ===================================================================================
...
// ===================================================================================
// Helper computed properties & event handlers
// ===================================================================================
...
const rawDynamicTicks = computed(() => {
  ...
  for (let i = startStep; i <= endStep; i++) {
    ...
    // Game time is offset from real time, so check window boundaries again
...
function onActionMouseDown(evt, track, action) {
  ...
  if (action.isLocked) {
    if (evt.button === 0) {
      store.selectAction(action.instanceId)
      ElMessage.warning({ message: 'This action is locked', duration: 1000, grouping: true })
      return
    }
  }
  ...
  if (isAltDown.value) {
    if (store.selectedActionId && store.selectedActionId !== action.instanceId) {
      ...
      if (!isShift) {
        if (isClickLeft) { alignMode = 'RL'; msg = 'Snapped to front' }
        else { alignMode = 'LR'; msg = 'Snapped to back' }
      } else {
        if (isClickLeft) { alignMode = 'LL'; msg = 'Left aligned' }
        else { alignMode = 'RR'; msg = 'Right aligned' }
      }
      ...
      if (success) {
        ...
        ElMessage.warning('Position unchanged')
      }
    }
    return
  }
    ...
    <div class="corner-placeholder">
      <div class="corner-controls">
        <div class="corner-button-row">
          ...
          <button class="mini-tool-btn" :class="{ 'is-active': connectionHandler.toolEnabled.value }" @click="store.toggleConnectionTool" title="Connection tool (Alt+L)">
    ...
    <el-dialog v-model="isWeaponSelectorVisible" title="Choose a weapon" width="600px" align-center class="char-selector-dialog" :append-to-body="true">
      <div class="selector-header">
        <div class="header-left-group">
          ...
          <button class="ea-btn ea-btn--glass-cut ea-btn--glass-cut-danger ea-btn--cut-left ea-btn--lift" :disabled="!currentWeaponForDialog" @click="removeWeapon" title="Remove current weapon">
            <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none">
              <path d="M3 6h18"></path><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
            </svg>
            Remove
    ...
    <el-dialog v-model="isEquipmentSelectorVisible" :title="`Select Equipment - ${equipmentSlotLabel}`" width="600px" align-center class="char-selector-dialog" :append-to-body="true">
      <div class="selector-header">
        <div class="header-left-group">
          ...
          <button class="ea-btn ea-btn--glass-cut ea-btn--glass-cut-danger ea-btn--cut-left ea-btn--lift" :disabled="!currentEquipmentForDialog" @click="removeEquipment" title="Remove current equipment">
            <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none">
              <path d="M3 6h18"></path><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
            </svg>
            Remove
          ...
          <div v-if="currentEquipmentForDialog?.level === 70" class="equipment-tier-picker">
            <span class="tier-label">Refine</span>
            <el-select :model-value="currentEquipmentTierForDialog" @update:model-value="setCurrentEquipmentTierForDialog" size="small" style="width: 92px">
              <el-option v-for="t in EQUIPMENT_REFINE_TIERS" :key="`tier_${t}`" :label="t === 0 ? 'Base' : `+${t}`" :value="t" />
            </el-select>
          </div>
        </div>
        <div class="element-filters">
          <button class="ea-btn ea-btn--glass-cut" :class="{ 'is-active': equipmentCategoryFilter === 'ALL' }" :style="{ '--ea-btn-accent': '#2dd4bf' }" @click="equipmentCategoryFilter = 'ALL'">All categories</button>
          <button class="ea-btn ea-btn--glass-cut" :class="{ 'is-active': equipmentCategoryFilter === '__UNCAT__' }" :style="{ '--ea-btn-accent': '#888' }" @click="equipmentCategoryFilter = '__UNCAT__'">Uncategorized</button>
        ...
        <div class="element-filters">
          <button class="ea-btn ea-btn--glass-cut" :class="{ 'is-active': equipmentLevelFilter === 'ALL' }" :style="{ '--ea-btn-accent': '#2dd4bf' }" @click="equipmentLevelFilter = 'ALL'">All levels</button>
        ...
        <template v-for="group in equipmentRosterByLevel" :key="group.level">
          ...
          <div class="roster-grid">
            <div v-for="eq in group.list" :key="eq.id" class="roster-card" @click="confirmEquipmentSelection(eq.id)">
              ...
              <div v-if="isEquipmentEquipped(eq.id)" class="in-team-tag weapon-equipped">Equipped</div>
            </div>
          </div>
        </template>
        <div v-if="equipmentRosterByLevel.length === 0" class="empty-roster">No matching equipment found</div>
      </div>
    </el-dialog>
  </div>
</template>
